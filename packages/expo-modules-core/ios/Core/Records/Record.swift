...
public protocol Record: Convertible {
  ...
  /**
   Reads the record's fields off a `JavaScriptObject`. The `@Field`-based default goes through
   reflection (`init()` + `update(withObject:)`); the `@Record` macro overrides it with a direct,
   statically-typed factory that reads each field by its declared type.
   */
  @JavaScriptActor
  static func from(object: borrowing JavaScriptObject, appContext: AppContext) throws -> Self

  /**
   Reads the record's fields from a `[String: Any]` dictionary. The `@Field`-based default goes
...
public extension Record {
  ...
  static func from(dictionary: Dict, appContext: AppContext) throws -> Self {
    let record = Self()
    try record.update(withDict: dictionary, appContext: appContext)
    return record
  }
