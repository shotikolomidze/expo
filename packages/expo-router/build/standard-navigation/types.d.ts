...
export type StandardNavigator<NavigatorOptions extends object, EventMap extends StandardNavigatorEventMapBase, NavigatorProps extends object> = ReturnType<typeof createStandardNavigator<NavigatorOptions, EventMap, NavigatorProps>>;
export type StandardUseNavigationBuilderOptions<State extends NavigationState, NavigatorOptions extends object, EventMap extends StandardNavigatorEventMapBase> = DefaultNavigatorOptions<ParamListBase, string | undefined, State, NavigatorOptions, EventMap & StandardNavigatorEventMapBase, any>;
export interface StandardNavigatorCreatePropsFactoryDeps<State extends NavigationState> {
    state: State;
    dispatch: (action: NavigationAction) => void;
    /**
     * The navigator's navigation helpers from `useNavigationBuilder`.
     *
     * @internal This is internal and may change (or be removed) between releases without notice. Prefer the
     * `state` and `actions` passed to `NavigatorContent`; reach for this only for capabilities the standard
     * contract does not expose (for example, `isFocused`).
     */
